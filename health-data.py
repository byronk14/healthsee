...
# all_zipcodes_info = search.by_population(lower=0, upper=9999999999999, returns=5)
...
class Facilities:
    ...
    def get_active_facilities(self, closest_zipcodes):
        ...
        '''
        facilities = self.df[self.df['ZIP_CD'].isin(x['zipcode'] for x in closest_zipcodes)]
    ...
    def insert_lat_long_dist_columns(self):
        ...
        '''
        self.active_fac_df['COMPLETE_ADR'] = self.active_fac_df['ST_ADR'] + ', ' + self.active_fac_df[
            'CITY_NAME'] + ', GA'
        self.active_fac_df['lat'], self.active_fac_df['long'], self.active_fac_df['Dist'] = zip(
            *self.active_fac_df['COMPLETE_ADR'].map(self.geolocate_lat_long))
...
def get_zipcode_coordinates(zipcode):
    ...
    df_zipcodes = pd.read_csv('Data/zip_codes_states.csv')
    df_zipcodes.set_index('zip_code', inplace=True)

    # only way to avoid key Error on Unknown index...
    if zipcode in df_zipcodes.index:
        lat = df_zipcodes.loc[zipcode]['latitude']
        lng = df_zipcodes.loc[zipcode]['longitude']
    else:
        lat = 0
        lng = 0
    # print(lat, lng)
...
def get_closest_zipcodes(zipcode, radius=5):
    ...
    zipcodes_enriched = search.by_coordinates(coordinates[0], coordinates[1], radius, returns=100)

    # add metrics from uszipcodes - median income, avg age etc...
    zipcodes_enriched = list(map(lambda x: {'zipcode': x.zipcode, 'population': x.population}, zipcodes_enriched))

    print(zipcodes_enriched)

    return zipcodes_enriched
...
def calculate_health_score(zipcode):
    ...
    total_population = sum(x['population'] for x in closest_zipcodes)
